<template>
  ...
  <div class="task-info-container statistical-template-container" style="overflow: hidden">
      ...
      <el-col>
        ...
        <div class="search-table">
          <el-table
            ...
            <el-table-column prop="infoId" label="关联ID"  show-overflow-tooltip> </el-table-column>
            <el-table-column prop="infoName" label="名称"  show-overflow-tooltip> </el-table-column>
    ...
    <el-dialog :title='isEdit?"编辑":"新增"' :visible.sync="dialogFormVisible">
      <el-form ref="editDialogForm" :model="addEditDialogForm" class="fSelect">
        ...
        <el-form-item label="公众号" :label-width="formLabelWidth">
          <el-button style="width:100%" @click="selectTemplate2">{{addSmsTemplate.id?addSmsTemplate.id+'：'+addSmsTemplate.pubName:'请选择公众号'}}</el-button>
        </el-form-item>
      </el-form>
    ...
    <!--选择公众号-->
    <el-dialog :title="'添加公众号'"  :visible.sync="addDialogParamVisible2">
      <el-button  class="selctParams" type="primary" style="margin-bottom: 15px;margin-right:15px;margin-left: 0px;" v-for="(item,index) in smsParamsList2" :key="item.id" @click.stop="addParamItem2(index,'editParam',$event)">
        {{ item.id }}：<em style="color:chartreuse">{{ item.pubName }}</em>
      </el-button>
      <div slot="footer" class="dialog-footer">
        <el-row>
          <el-col :span="14">
...
<script>
  ...
  import { wxListOrgInfos , deleteOrgBindingInfo , revokeOrgBindingInfo  , addOrgBindingInfo , wxListInfos} from '@/api/assist-api'
  ...
  export default {
    ...
    methods: {
      handleCurrentChangeParam2(val){
        let _self = this;
        let data = {};
        wxListInfos(data).then(function (res) {
      ...
      selectTemplate2(){
        ...
        let dataParam = {};
        wxListInfos(dataParam).then(function (res) {
          _self.smsParamsList2 = res.data;
          _self.pageparam2 = {
            pageSize:12,
            total:res.total,
      ...
      addParamItem2(index,formName,ev){
        let _self = this;
        _self.addSmsTemplate.pubName = _self.smsParamsList2[index].pubName;
        _self.addSmsTemplate.id = _self.smsParamsList2[index].id;
      ...
      submitForm(formName){
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if(_self.isEdit){
              let data = {"infoId":_self.addSmsTemplate.id,"orgCode":_self.orgCode,"orgId":_self.orgId	};
              addOrgBindingInfo(data).then(function (res) {
                _self.$message({
                  type: 'success',
                  message: '编辑成功!'
                });
                _self.tableData[_self.Editindex].cityId = _self.addSmsTemplate.id;
              ...
              addOrgBindingInfo(data).then(function (res) {
                ...
                wxListOrgInfos(data).then(function (res) {
      ...
      handleStop(index,row){
        ...
        if(_self.tableData[index].status===1){
          this.$confirm('此操作将禁用【' + row.infoName + '】, 是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            deleteOrgBindingInfo(data).then(function(res){
              if (res.success) {
                _self.$message({
                  type: 'success',
                  message: '禁用成功!'
          ...
          this.$confirm('此操作将激活【' + row.infoName + '】, 是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            revokeOrgBindingInfo(data).then(function(res){
              _self.$message({
                type: 'success',
                message: '激活成功!'
              });
