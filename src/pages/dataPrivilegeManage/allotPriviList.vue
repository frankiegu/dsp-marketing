<template>
    <div class="allot-privi-list-container">
      <el-row>
        <el-col>
          <div class="search-container">
            <el-button type="success" class="search-add pull-left"  @click="$router.push({path: '/home/privilegelist'})">返回</el-button>
      ...
      <el-dialog title="编辑数据权限"  size="tiny" :visible.sync="privilegeDialog.flag">
...
<script>
    import { privilegeSave,allotList } from '@/api/dataPrivilege-api'
      ...
      methods: {
        ...
        handleEdit(item){
          let _promise = new Promise((resolve, reject)=>{
            this.handleOwnerList(item.ownerType);
            resolve();
          }).then(()=>{
            this.privilegeDialog.flag = true;
            this.privilegeDialog.isEdit = true
            this.privilege={
              id : item.id,
              belongDataId : item.belongDataId,
              privilegeArr : this.handleTransConvert(item.privilege),
              privilege:item.privilege,
              ownerType : Number(item.ownerType),
              privilegeOwners : Number(item.privilegeOwner),
            };
          });
        },
        handleView(){},
          ...
          this.$refs[formName].validate((valid) => {
            if (valid) {
              let _data = {
                ...
                belongDataId : this.privilege.belongDataId,
                privilege:this.privilege.privilege,
                privilegeOwners : [this.privilege.privilegeOwners],
                ownerType : this.privilege.ownerType
              }
              privilegeSave(_data).then( data => {
                this.$message({
                  showClose: true,
                  message: '权限分配成功！',
                  type: 'success'
        ...
        handleTransConvert(val){
           let _privilegeArr = [];
           let _bitArr = parseInt(val).toString(2).split('').reverse();
           _bitArr.forEach( (item,index) => {
               parseInt(item) && _privilegeArr.push(Math.pow(2,index))
           });
           return _privilegeArr;
        },
