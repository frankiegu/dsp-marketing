<template>
  <div class="passageway-info-container" style=" background-color: #fff">
      ...
      <el-col>
        ...
        <div class="search-table">
          <el-table
            ...
            <el-table-column prop="cityId" label="城市ID" width="120" show-overflow-tooltip> </el-table-column>
            <el-table-column prop="cityName" label="城市名称" width="250" show-overflow-tooltip> </el-table-column>
            <el-table-column prop="mainPid" label="合作商ID" width="100" show-overflow-tooltip> </el-table-column>
            <el-table-column prop="ruleId" label="规则ID" width="100" show-overflow-tooltip> </el-table-column>
            <el-table-column prop="stepNo" label="入资步骤" width="100" show-overflow-tooltip>
              <template slot-scope="scope">
                {{ stepList[scope.row.stepNo] }}
              </template>
            </el-table-column>
    ...
    <el-dialog :title='isEdit?"新增":"编辑"'  :visible.sync="dialogFormVisible">
      <el-form :model="addSmsTemplate" ref="addSmsTemplate">
        ...
        <el-form-item label="规则ID" :label-width="formLabelWidth">
            <el-button @click="selectTemplate"  style="width:100%;">{{addSmsTemplate.ruleId?addSmsTemplate.ruleId:'请选择规则ID'}}</el-button>
        </el-form-item>
        <el-form-item label="入资步骤" :label-width="formLabelWidth">
          <el-select v-model="addSmsTemplate.stepNo" style="width:100%;" placeholder="请选择入资步骤" clearable filterable >
            <el-option :label="item" :value="key" v-for="(item,key) in stepList" :key="key"></el-option>
          </el-select>
    ...
    <el-dialog :title="'添加参数'"  :visible.sync="addDialogParamVisible">
      <el-tooltip class="item" effect="dark" :content="item.partnerName" placement="top" v-for="(item,index) in smsParamsList" :key="item.id" :disabled="item.partnerName?false:true">
      <el-button  class="selctParams" type="primary" style="margin-bottom: 15px;margin-right:15px;margin-left: 0px;" @click.stop="addParamItem(index,'editParam',$event)">
        {{ item.id }}<em style="color:chartreuse"></em>
      </el-button>
      </el-tooltip>
      <div slot="footer" class="dialog-footer">
        <el-row>
...
<script>
  ...
  import { relationList , relationAddOrModify , relationDelete , ruleList ,listPartners} from '@/api/capital-api';
  ...
  export default {
    ...
    methods: {
      ...
      handleCurrentChangeParam2(val){
        let _self = this;
        let data2 = {"status":1,"currPage":val,isAdmin:1,"prePageResult":10 };
      ...
      selectTemplate2(){
        ...
        let data2 = {"status":1,"currPage":0,isAdmin:1,"prePageResult":10 };
        listPartners(data2).then(function (res) {
          _self.smsParamsList2 = res.data;
          _self.pageparam2={
            pageSize: 10,
      ...
      selectTemplate(){
        ...
        ruleList(dataParam).then(function (res) {
          _self.smsParamsList = res.data;
          _self.pageparam = {
            pageSize:12,
            total:res.total,
      ...
      addParamItem(index,formName,ev){
        let _self = this;
        this.addSmsTemplate.ruleId = _self.smsParamsList[index].id;
      ...
      searchEvent(){
        ...
        _self.loading2 = true;
        relationList(data).then(function (res) {
      ...
      handleEdit(formName, index, row) {
        ...
        this.addSmsTemplate.partnerId = this.tableData[index].mainPid;
        this.addSmsTemplate.ruleId = this.tableData[index].ruleId;
        this.addSmsTemplate.stepNo = this.tableData[index].stepNo +'';
      ...
      handleStop(index, row){
        ...
        if(row.status){
          this.$confirm('此操作将禁用【' + row.id + '】, 是否继续?', '提示', {
            ...
            relationDelete(data).then(function (res) {
              _self.$message({
                type: 'success',
                message: '禁用成功!'
              });
          ...
          if (valid) {
            ...
            if(_self.isEdit){
              let data = {
                ...
                mainPid:_self.addSmsTemplate.partnerId,
                ruleId:_self.addSmsTemplate.ruleId,
                stepNo:_self.addSmsTemplate.stepNo,
                cityId:_self.addSmsTemplate.cityId,
                cityName:_self.addSmsTemplate.cityName,
                user:this.account,
                uId:"admin"//此处写死 admin
              };
              relationAddOrModify(data).then(function(res){
                ...
                _self.tableData[_self.editindex].ruleId = _self.addSmsTemplate.ruleId;
                _self.tableData[_self.editindex].stepNo = _self.addSmsTemplate.stepNo;
              ...
              relationAddOrModify(data).then(function(res){
                ...
                _self.loading2 = true;
                relationList(JSON.stringify(data)).then(function (res) {
                  _self.loading2 = false;
                  _self.tableData = res.data;
                  _self.page = {
    ...
    created() {
      ...
      _self.loading2 = true;
      cityView({}).then((res)=>{
        _self.options = _self.getCityDataWithoutXian(res)
      });
      relationList(data).then(function (res) {
        _self.loading2 = false;
        _self.tableData = res.data;
